...
class AllJobs extends Component {
  ...
  onGetJobDetails = async () => {
    ...
    const jobsApiUrl = `https://apis.ccbp.in/jobs?employment_type=${type}&minimum_package=${radioInput}&search=${searchInput}`
    const optionsJobs = {
      headers: {
        Authorization: `Bearer ${jwtToken}`,
      },
    ...
    const response = await fetch(jobsApiUrl, optionsJobs)
  ...
  onGetInputOption = event => {
    ...
    if (checkboxInputs.includes(event.target.id)) {
      ...
      this.setState({checkboxInputs: updatedList}, this.onGetJobDetails)
    } else {
      this.setState(
        prevState => ({
  ...
  onGetProfileView = () => {
    ...
    return (
      <div className="profile-container">
        ...
        <p className="profile-desccription">{shortBio}</p>
      </div>
    )
  }
